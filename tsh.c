 ...
 * 
 * Melecio Perea mepe6365@colorado.edu
 */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
void eval(char *cmdline) {
    ...
    {
    	sigaddset(&frontline, SIGCHLD);
	sigemptyset(&frontline);
    	sigprocmask(SIG_BLOCK, &frontline, NULL); 
    	if((pid = Fork()) == 0){
    		setpgid(0, 0);
                sigprocmask(SIG_UNBLOCK, &frontline, NULL); 
    	...
    	if (!framework){
    		...
    		sigprocmask(SIG_UNBLOCK, &frontline, NULL);
            int jobIdentity = get_jid_from_pid(pid);
    		printf("[%d] (%d) %s", jobIdentity, pid, cmdline);
    	}
    }
    return;
}
...
void do_bgfg(char **argv){
    ...
    if ((argv[1][0] == 'J' || !atoi(argv[1]) == 0 )? 0 : 1){
        printf("%s : argument must be a PID or Jobid \n", argv[0]);
        return;
    }

    int process = (argv[1][0] == 'J' ? 0 : 1);
    struct job_t *followup;
 
    if (process){
        followup = getprocessid(jobs, (pid_t) atoi(argv[1]));
        if (followup == NULL){
        ...
        followup = getjobid(jobs, atoi(&argv[1][1]));
        if (followup == NULL){
    ...
    if(strcmp(argv[0], "bg") == 0){
        followup->state = BG;
        printf("[%d] (%d) %s", followup->jid, followup->pid, followup->cmdline);
        kill(-followup->pid, SIGCONT);
    }
    else{
        followup->state = FG;
        kill(-followup->pid, SIGCONT);
...
{
    pid_t pid;
    int current, possibleID;

    while((pid = waitpid(-1, &current, WNOHANG | WUNTRACED)) > 0){
        possibleID = get_jid_from_pid(pid);
	...
	if (WIFEXITED(current)){
            ...
            printf("Job [%d] (%d) stopped by signal %d\n", possibleID, (int) pid, WSTOPSIG(current));
        }        

        else if(WIFSIGNALED(current)) {
            removejob(jobs, pid); 
            printf("Job [%d] (%d) terminated by signal %d\n", possibleID, (int) pid, WTERMSIG(current));
...
void sigalrm_handler(int sig){
    int x;
    for (x = 0; x < MAXJOBS; x++){
        pid_t pid = jobs[x].pid;
        if (jobs[x].state == BG && pid != 0){
            kill(-pid, SIGINT);
        }
    }
    return;
